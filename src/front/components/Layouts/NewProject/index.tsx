...
class NewProject extends BasePage<IProps, IState> {
            ...
            <DefaultTemplate title={"Create Project"}>
                <div className={classes["root"]}>
                    ...
                    <div>
                        ...
                        <Button text="Create project" icon={SignUpIcon} onClick={this.handleSubmit} />
                    </div>
                </div>
            </DefaultTemplate>
        );
    ...
    private async handleSubmit() {
        if (this.checkName(this.state.name)) {
            const uuid : string = (await Project.getInstance().postProject({title: this.state.name, network: this.state.network})).uuid;
